...
var app = express();
var url = 'mongodb://root:root@ds031995.mlab.com:31995/tutorial8';
app.use(cors());
...
var insertDocument = function(db, data, callback) {
    ...
    db.collection('abc').insertOne( data, function(err, result) {
        if(err)
        {
...
var deleteUser = function(db, data, callback) {
    ...
    db.collection('abc').deleteOne( {"email": data.email}, function(err, result) {
        if(err)
        {
...
var updateDocument = function(db, data, callback) {
    db.collection('abc').updateOne({"email":email},{$set:item}, function(err, result) {
        if (err) {
